...
The complete security checklist by Taha Jaffri. Drop it in your project root. Ship securely every time.

## 1. Secrets and Environment Variables
Never expose secrets in frontend code.
* Every API key, token, database URL, and private config lives in `.env` files only.
* `.env` files must always be listed in `.gitignore`.
...
* AI proxy endpoints: 10 requests per minute per user.
...
* Use parameterized queries or ORM methods. Never construct queries via string concatenation with user data.
