    ...
    <>
      <header 
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
          scrolled 
            ? 'bg-[#0B1B3A] backdrop-blur-md border-b border-white/10 shadow-md h-20' 
            : 'bg-[#0B1B3A] backdrop-blur-md border-b border-white/5 h-24'
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-4">
            <a 
              ...
              className="px-5 py-2.5 rounded-full border border-[#C9A227] bg-[#C9A227] text-[#0B1B3A] text-xs font-bold tracking-wider hover:bg-transparent hover:text-white hover:border-white active:scale-95 duration-300 flex items-center gap-2 group shadow-md"
            >
              ENQUIRE NOW
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="absolute top-full left-0 w-full bg-[#0B1B3A] border-b border-white/10 shadow-2xl overflow-hidden md:hidden"
            >
              <div className="px-6 py-8 flex flex-col gap-5 bg-[#0B1B3A]">
                {navLinks.map((link) => {
                  const isActive = pathname === link.path;
                  return (
                    <Link
                      ...
                      className={`text-xs font-bold tracking-widest py-2 border-b border-white/10 transition-colors hover:text-[#C9A227] ${
                        isActive ? 'text-[#C9A227]' : 'text-slate-200'
                      }`}
                    >
                      {link.label}
                    </Link>
                ...
                <a 
                  ...
                  rel="noopener noreferrer"
                  onClick={() => setMobileMenuOpen(false)}
                  className="mt-4 w-full py-3 bg-[#C9A227] text-[#0B1B3A] rounded-xl font-bold text-xs tracking-widest text-center uppercase shadow-lg hover:bg-amber-400 duration-300 flex items-center justify-center gap-2"
                >
